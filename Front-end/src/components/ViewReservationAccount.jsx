...
class ViewReservationAccount extends Component {
    ...
    ReturnVehicle(vrid) {
        VehicleService.returnVehicle(vrid).then((res) => {
            alert(res.data);
            VehicleService.getVehicleByVRId(vrid).then((rest) => {
                this.state.vehicleid = rest.data;
                this.props.history.push(`/vehicle/view/${this.state.vehicleid}`);
                                ...
                                vrReservation =>
                                    <tr key={vrReservation.id}>
                                        ...
                                        <td className='row'>
                                            ...
                                            <div className='btn-group'>
                                                { this.state.flagPickup ? <button style={{ marginLeft: "10px" }} onClick={() => {this.PickupVehicle(vrReservation.id);}} className="btn btn-success btn-sm"> Pickup Vehicle</button> 
                                                                        : <></>}
                                                { this.state.flagReturn ? <button style={{ marginLeft: "10px" }} onClick={() => this.ReturnVehicle(vrReservation.id)} className="btn btn-primary btn-sm"> Return Vehicle</button> 
                                                                        : <></>}
                                            </div>
                                        </td>
